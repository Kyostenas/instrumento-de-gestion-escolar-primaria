...
export class BootstrapDropdownDirective implements OnInit, OnDestroy {
    ...
    @Input('posicion_dropdown') posicion: 'up' | 'start' | 'end' | 'down' =
        'down';
    @Input('definir_scroll_maximo_automatico')
    definir_scroll_maximo_automatico = false;
    ...
    @Input('id_elemento_a_enfocar_cuando_padre_destruido')
    id_enfoque_padre_destruido!: string;
    @Output('dropdown_destruido')
    emisor_dropdown_destruido: EventEmitter<void> = new EventEmitter();
    @Output('mostrando_dropdown')
    emisor_mostrando_dropdown: EventEmitter<boolean> = new EventEmitter();
    @Output('id_dropdown') emisor_id_dropdown: EventEmitter<string> =
        new EventEmitter();
    @Output('id_boton_cerrado') emisor_id_boton_cerrado: EventEmitter<string> =
        new EventEmitter();
    ...
    on_click(event: Event) {
        if (this.mostrar_con_click) {
            ...
            const ESTA_DENTRO_DROPDOWN =
                this.control_dropdowns.elemento_esta_dentro_de_dropdown(
                    TARGET as Node
                );
            const ESTA_DENTRO_PADRE = this.elemento_padre.contains(
                event.target as Node
            );
            const ESTA_EN_ESTE_DROP_DOWN = this.dropdown?.contains(
                event.target as Node
            );
    ...
    on_document_click(event: MouseEvent) {
        ...
        const NIVEL_PARENTESCO =
            this.utiles.obtener_nivel_de_descendencia_entre_nodos_html(
                <HTMLElement>this.dropdown,
                event.target as HTMLElement
            );
        if (
            TARGET.id !== this.id_dropdown &&
            TARGET.id !== this.id_boton_cerrado
        ) {
            if (
                this.solo_ocultar_con_boton &&
        ...
        if (
            ...
            (NIVEL_PARENTESCO < 0 && !ESTA_DENTRO_PADRE)
        ) {
            this.ocultar();
        }
    }
    ...
    construir_dropdown() {
        ...
        `
            ...
            this.vista_embebida = this.plantilla_dropdown.createEmbeddedView(
                this.contexto_plantilla
            );
    ...
    private encontrar_todos_los_elementos_shadow_root(
        ...
        elemento.childNodes.forEach((child: any) => {
            if (child.shadowRoot) {
                elementos.push(
                    ...this.encontrar_todos_los_elementos_shadow_root(
                        child.shadowRoot,
                        query
                    )
                );
    ...
    private encontrar_todos_los_elementos_solo_query_selector(
        elemento: HTMLElement,
        query: string
    ) {
        const elementos = Array.from(
            (<HTMLElement>this.dropdown).querySelectorAll(query)
        );
        return elementos;
    }
    ...
    private obtener_hijos(elemento: HTMLElement, query: string) {
        return [
            ...
            ...this.encontrar_todos_los_elementos_solo_query_selector(
                elemento,
                query
            )
        ];
    ...
    private aplicar_estilos_personalizados() {
        const elementosHR = this.obtener_hijos(
            <HTMLElement>this.dropdown,
            'hr.hr-completo'
        );
        const elementosCompletos = this.obtener_hijos(
            <HTMLElement>this.dropdown,
            '.recuadro-completo'
        );
    ...
    private posicionar_dropdown(noOcultar: boolean = false) {
        ...
        if (!this.posicionado_por_salir_de_pantalla) {
            switch (posicionAValorar) {
                case 'up':
                    this.top_dropdown =
                        posicionHost.top + scrollYAxis - alturaFLotante;
        ...
        if (!!this.dropdown) {
            try {
                this.renderer.setStyle(
                    this.dropdown,
                    'top',
                    `${this.top_dropdown}px`
                );
                this.renderer.setStyle(
                    this.dropdown,
                    'left',
                    `${this.left_dropdown}px`
                );
    ...
    private relocalizar_por_salir_de_pantalla() {
        setTimeout(() => {
            try {
                const rectFlotante = (<HTMLElement>(
                    this.dropdown
                ...
                if (rectFlotante.right > viewPortWidth) {
                    puntoOverflow = 'end';
                    this.left_dropdown -=
                        rectFlotante.right - viewPortWidth - 1;
