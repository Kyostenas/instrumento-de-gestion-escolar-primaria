...
export class ControlBreadcrumbsService {
    ...
    private _menu_actual_subject = new BehaviorSubject<DESCRIPCION_MENU>(
        this.menu_actual
    );
    private _ruta_menus_actual_subject = new BehaviorSubject<
        DESCRIPCION_MENU[]
    >(this._ruta_menus_actual);
    private _url_actual_subject = new BehaviorSubject<string[]>(
        this.url_actual
    );
    private _conjunto_sub_menus_disponibles_subject = new BehaviorSubject<
        DESCRIPCION_MENU[]
    >(this._conjunto_sub_menus_disponibles);

    estado_menu_actual$ = this._menu_actual_subject.asObservable();
    estado_ruta_menus_actual$ = this._ruta_menus_actual_subject.asObservable();
    estado_url_actual$ = this._url_actual_subject.asObservable();
    ...
    estado_conjunto_sub_menus_disponibles$ =
        this._conjunto_sub_menus_disponibles_subject.asObservable();
        ...
        this.subscripcion_router = this.router.events
            ...
            .subscribe({
                next: (event) => {
                    ...
                    this._url_actual_subject.next(this.url_actual);
    ...
    private obtener_menus_actuales() {
        ...
        for (
            let iPasoRuta = 0;
            iPasoRuta < URL_ACTUAL_SIN_LAYOUT.length;
            iPasoRuta++
        ) {
            const UN_ELEM_URL = URL_ACTUAL_SIN_LAYOUT[iPasoRuta];
            let menu_correspondiente: DESCRIPCION_MENU = <DESCRIPCION_MENU>(
                conjuntoMenusActual.find((menu) => menu.link === UN_ELEM_URL)
            );
        ...
        this._conjunto_sub_menus_disponibles =
            this.menu_actual?.sub_menus || [];
        this._ruta_menus_actual = rutaDeMenus;
        this._ruta_menus_actual_subject.next(this._ruta_menus_actual);
        ...
        this._conjunto_sub_menus_disponibles_subject.next(
            this._conjunto_sub_menus_disponibles
        );
    }
}
